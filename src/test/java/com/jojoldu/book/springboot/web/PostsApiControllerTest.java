...
public class PostsApiControllerTest {
    ...
    @Test
    @WithMockUser(roles="USER")
    public void ID로_조회된다() throws Exception {
        // given
        ...
        mvc.perform(get(url).contentType(MediaType.APPLICATION_JSON_UTF8)).andExpect(status().isOk()).andReturn().getResponse().getContentAsString().contains("title");
    ...
    @WithMockUser(roles="USER")
    public void 삭제된다() throws Exception {
        // given
        Posts posts = postsRepository.save(Posts.builder().title("title").content("content").author("author").build());
        ...
        postsService.findById(posts.getId());
    }
}
